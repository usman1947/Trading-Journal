...
export async function getWeeklyStats(
  ...
  const totalPnl = trades.reduce((sum, t) => sum + (t.result ?? 0) - (t.commission ?? 0), 0);
  const winRate = nonBETrades.length > 0 ? (winningTrades.length / nonBETrades.length) * 100 : 0;
  const executionRate = trades.length > 0 ? (passingTrades.length / trades.length) * 100 : 0;
  ...
  // Largest win/loss
  const largestWin =
    winningTrades.length > 0 ? Math.max(...winningTrades.map((t) => t.result ?? 0)) : 0;
  const largestLoss =
    losingTrades.length > 0 ? Math.min(...losingTrades.map((t) => t.result ?? 0)) : 0;
  ...
  trades.forEach((t) => {
    ...
    dayPnl.set(day, (dayPnl.get(day) ?? 0) + (t.result ?? 0) - (t.commission ?? 0));
  });
  ...
  const bestDay =
    dayEntries.length > 0
      ? dayEntries.reduce(
          (best, [day, pnl]) => (pnl > (best?.pnl ?? -Infinity) ? { day, pnl } : best),
  ...
  const worstDay =
    dayEntries.length > 0
      ? dayEntries.reduce(
          (worst, [day, pnl]) => (pnl < (worst?.pnl ?? Infinity) ? { day, pnl } : worst),
          null as { day: string; pnl: number } | null
        )
      : null;
  ...
  const avgConfidence =
    tradesWithConfidence.length > 0
      ? tradesWithConfidence.reduce((sum, t) => sum + (t.confidenceLevel ?? 0), 0) /
  ...
  const mostCommonMistake =
    mistakeCounts.size > 0
      ? (Array.from(mistakeCounts.entries()).reduce(
          (most, [mistake, count]) => (count > (most?.count ?? 0) ? { mistake, count } : most),
  ...
  const strategyPerformance = Array.from(strategyMap.entries()).map(([name, stats]) => ({
    name,
    trades: stats.trades,
  ...
  trades.forEach((t) => {
    if (t.setup) {
      const name = t.setup;
      const existing = setupMap.get(name) ?? {
        trades: 0,
        wins: 0,
        pnl: 0,
        totalR: 0,
  ...
  const topSymbol =
    symbolPnl.size > 0
      ? Array.from(symbolPnl.entries()).reduce(
          (top, [symbol, pnl]) => (pnl > (top?.pnl ?? -Infinity) ? { symbol, pnl } : top),
  ...
  const timeMap = new Map<string, { trades: number; wins: number; pnl: number }>();
  trades.forEach((t) => {
  ...
  const timeOfDayPerformance = Array.from(timeMap.entries()).map(([period, stats]) => ({
    period,
    trades: stats.trades,
    winRate: stats.trades > 0 ? (stats.wins / stats.trades) * 100 : 0,
    pnl: stats.pnl,
